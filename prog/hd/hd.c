...
void hd_read(Message* msg){
    ...
    do{
        ...
        memcpy((void*)get_process_phy_mem(msg->src_pid, (uint32_t)buf_ptr), hdbuf, copy_len);
...
void hd_write(Message* msg){
    ...
    do{
        uint32_t copy_len = remain_len < SECTOR_SIZE ? remain_len : SECTOR_SIZE;
        memcpy(hdbuf, (void*)get_process_phy_mem(msg->src_pid, (uint32_t)buf_ptr), copy_len);
        wait_for_hd_busy();
